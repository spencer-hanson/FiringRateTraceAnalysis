...
def main():
    ...
    plt.title(f"Number of spikes per trial of unit {unit_num}")
    plt.plot(np.sum(unit_spikes, axis=1))
    plt.hlines(0, 0, unit_spikes.shape[0], color="red")
    plt.show()

    plt.title(f"Mini spikes raster of unit {unit_num}")
    [plt.plot(s, color="blue", alpha=.1) for s in unit_spikes[:50]]  # TODO only first 50 trials?
    plt.show()

    plt.title(f"Mini mean responses of unit {unit_num}")
    [plt.plot(w, color="blue", alpha=.1) for w in sess.units()[unit_num][:100]]
    plt.show()
